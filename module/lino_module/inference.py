...
class RecurrentInference():
    """再起的に推論を行うクラス"""
    def __init__(self, model, data, seq, d_model, dilation, src_tgt_seq,
    ...
    @classmethod
    def inference(self, model: object, src: ndarray, tgt: ndarray) -> ndarray:
        """推論関数
        """
...
# for UniTransformer
class UniRecurrentInference():
    """再起的に推論を行うクラス"""
    def __init__(self, model, data, seq, dilation, src_tgt_seq,
                 step_num, daily, weekday, weekly, monthly, scaler):
        """ Initializer
        引数:
            model: 訓練済みモデル
        ...
        self.df: Optional[DataFrame] = None
        self.inferenced: Optional[Series] = None
        self.input_seq: Optional[ndarray] = None
        self.latest_index: Optional[Timestamp] = None
        self.latest_data: Optional[float] = None
    ...
    def predict(self, freq: int) -> Series:
        ...
        """
        for _ in range(freq):
            self.input_seq = self.df.values[-self.seq:]
            src, tgt = src_tgt_split(self.input_seq.T, *self.src_tgt_seq)
            output = self.inference(self.model, src, tgt).reshape(-1)
            scaled = output[-self.step_num:]
            inversed = self.scaler.inverse_transform(scaled.reshape(-1, 1))
            inversed = inversed.reshape(-1)
